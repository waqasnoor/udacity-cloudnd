...
router.patch("/:id", requireAuth, async (req: Request, res: Response) => {
  ...
  const feedItem: FeedItem = await FeedItem.findByPk(id);
  if (caption) feedItem.caption = caption;
  if (fileName) feedItem.url = fileName;
  const updatedFeedItem = await feedItem.save();

  res.status(200).send(updatedFeedItem);
});
